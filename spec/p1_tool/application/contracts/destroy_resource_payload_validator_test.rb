...
describe P1Tool::Application::Contracts::DestroyResource::PayloadValidator do
  let(:validator) { P1Tool::Application::Contracts::DestroyResource::PayloadValidator.new }
  let(:payload) { fixture_json('runtime', 'destroy_resource_input.json').fetch('payload') }
  ...
  it 'normalizes valid payload with string keys' do
    ...
    assert_equal 'Condition', result.dig(:resource, :resource_type)
  end
