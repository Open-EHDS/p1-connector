...
Ten rozdzial opisuje operacje `destroy_resource`.

## Cel operacji

Operacja usuwa wskazany zasob z P1.
...
| `operation_kind` | string | tak | Dla tej operacji musi byc rowne `destroy_resource`. |
...
| Pole | Typ | Wymagane | Uwagi |
| --- | --- | --- | --- |
| `resource_type` | string | tak | Jedna z wartosci: `Encounter`, `Procedure`, `Condition`, `Provenance`. |
| `resource_id` | string | tak | Id zasobu w P1. |

## Zakres obslugiwanych danych

Operacja obsluguje usuwanie dla typow:
...
`Patient` nie jest wspierany przez te operacje.
...
| Pole | Typ | Uwagi |
| --- | --- | --- |
| `resource_type` | string | Typ usunietego zasobu. |
...
```json
{
  ...
  "error": {
    "code": "invalid_input",
    "message": "Destroy resource payload validation failed",
    "category": "input"
  },
  "details": {
    "validation_errors": {
      "resource": {
        "resource_type": [
          "must be one of: Encounter, Procedure, Condition, Provenance"
        ]
      }
    }
  }
...
- [examples/destroy_resource.json](examples/destroy_resource.json)
...
bin/p1-tool run-once \
  ...
  --input docs/demo/examples/destroy_resource.json \
  --output tmp/destroy_resource.result.json
```

### `watch`

```bash
cp docs/demo/examples/destroy_resource.json <inbox>/destroy_resource.json
```

W trybie `watch` wynik zostanie zapisany jako:

```text
<results>/destroy_resource.json.result.json
```
